...
/**
 * Polling outbox publisher: picks up unpublished rows and pushes them to
 * Kafka, marking each published only after a synchronous send succeeds. A
 * crash between "sent" and "marked published" causes a harmless duplicate
 * delivery, not a lost one — which is why every consumer here is idempotent.
 */
@Component
public class OutboxPublisher {
    ...
    public OutboxPublisher(
            ...
            @Value("${spicyeat.kafka.order-topic}") String topic
    ) {
